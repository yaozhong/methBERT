...
"""
2020/05/07 Evaluation model the binary DL models
"""
import torch
import torch.nn.functional as F
import time,tqdm,os,math
from sklearn.metrics import roc_curve, auc, roc_auc_score, f1_score, accuracy_score, confusion_matrix, matthews_corrcoef, average_precision_score
import numpy as np
...
def get_group_pred(mList, threshold=0.5):
...
def group_eval(align_list, output_list, gold_list, label=1, cov_threshold=5):
	...
	for pos in position_dic.keys():
		if len(position_dic[pos]) < cov_threshold:
			continue

		pred_list.append(get_group_pred_rate(position_dic[pos]))
		gold_list.append(label)

	return pred_list, gold_list


# 20210501 group evaluation adjust
def group_eval_new(align_list, output_score, gold_list, label=1, threshold=5):

	position_dic, gold_dic = {}, {}

	for (a, pred_score, gold) in zip(align_list, output_score, gold_list):
		...
		# prediction
		if align not in position_dic.keys():
			position_dic[align] = [pred_score]
		else:
			position_dic[align].append(pred_score)
	...
	## evaulations
	gold_list, pred_score = [],[]

	#for pos in tqdm.tqdm(position_dic.keys(), total=len(position_dic.keys())):
	for pos in position_dic.keys():
		if len(position_dic[pos]) < threshold:
			continue
		#pred_list.append(get_group_pred_new(position_dic[pos]))
		pred_score.append(np.mean(position_dic[pos]))
		gold_list.append(label)

	return pred_score, gold_list
...
def evaluate_event(net, criterion, device, val_generator, useSEQ, use_sim, generator=None, permute=False, vis=False, siamese=False):
	...
	with torch.no_grad():
		...
		#for i, val_data in tqdm.tqdm(enumerate(val_generator, 0), total=len(val_generator)):
			...
			pred_list.extend(torch.argmax(outputs, dim=1).cpu().numpy())
			# updated 20210501
			pred_list_score.extend(F.softmax(outputs, dim=1).cpu().numpy()[:,1]) 
			# added cpu() here
		...
		# Calculation of AUC
		#auc_score_old = roc_auc_score(gold_list, pred_list)
		auc_score = roc_auc_score(gold_list, pred_list_score)
		#print("[!!!!] - auc checking old [%.4f] vs. correct: [%.4f]" %(auc_score_old, auc_score))
		...
		#auprc_old = average_precision_score(gold_list, pred_list)
		auprc = average_precision_score(gold_list, pred_list_score)
		#print("[!!!!] - auprc checking old [%.4f] vs correct [%.4f]:" %(auprc_old, auprc))

		print(" @ <Read-level evaluation>")
		print(" |* Total test input-instance [%d]" %(n_sample))
		print(" |= tn=%d, fp=%d, fn=%d, tp=%d" %(tn, fp, fn, tp))
		print(" |= AUC=%.4f, Precision=%.4f, Recall=%.4f" %(auc_score, precision, recall))
		print(" |= ACC=%.4f, F1=%.4f, MCC=%.4f, AUPRC=%.4f" %(acc, f1, mcc, auprc))

	# Group evaluation 
	## updated new version for the comparision
	cov_thres = 1
	...
	# add the new implementation of the group predcition
	ps,g = group_eval_new(align_list, pred_list_score, gold_list, 1, cov_thres)
	group_pred_score.extend(ps)
	group_pred_list.extend([int(x>0.5) for x in ps])
	group_gold_list.extend(g)
	...
	# add the new implementation of the group predcition
	ps,g = group_eval_new(align_list, pred_list_score, gold_list, 0, cov_thres)
	group_pred_score.extend(ps)
	group_pred_list.extend([int(x>0.5) for x in ps])
	group_gold_list.extend(g)

	#group_auc_score_old = roc_auc_score(group_gold_list, group_pred_list)
	group_auc_score = roc_auc_score(group_gold_list, group_pred_score)
	#print("[!!!!] - auc checking GROUP old [%.4f] vs. correct: [%.4f]" %(group_auc_score_old, group_auc_score))
	...
	group_auprc = average_precision_score(group_gold_list, group_pred_score)
	...
	print(" |= group_ACC=%.4f, group_F1=%.4f, group_MCC=%.4f, group_auprc=%.4f" %(group_acc, group_f1, group_mcc, group_auprc))
	...
	return	valid_loss, accuracy, auc_score, precision, recall, sensitivity, specificity, auprc

# 20210509 revised
# 20210609 additional revision
def evaluate_event_cached(net, criterion, device, val_generator, useSEQ, use_sim, generator=None, permute=False, vis=False, siamese=False):

	# data unpack
	data_X, data_Y, align_list, n_sample = val_generator
	...
	ps,g = group_eval(align_list, pred_list, gold_list, 1, cov_thres)
	#group_pred_list.extend([int(x>0.9) for x in ps])
	group_pred_list.extend([int(x>0.5) for x in ps])
	...
	ps,g = group_eval(align_list, pred_list, gold_list, 0, cov_thres)
	#group_pred_list.extend([int(x>0.1) for x in ps])
	group_pred_list.extend([int(x>0.5) for x in ps])
...
def evaluate_bert(net, criterion, device, val_generator, useSEQ, use_sim):
	...
	with torch.no_grad():
		...
		#auc_score_old = roc_auc_score(gold_list, pred_list)
		auc_score = roc_auc_score(gold_list, pred_list_score)
		#print("[!!!!] - auc checking old vs correct:", auc_score_old, auc_score)
		...
		#auprc_old = average_precision_score(gold_list, pred_list)
		auprc = average_precision_score(gold_list, pred_list_score)
		#print("[!!!!] - auprc checking old vs correct:", auprc_old, auprc)
