...
def detection(net, data_gen, device, model, model_save_path, eval_mode, use_sim=False, unseg=False, generator=None, permutate=False):
	...
	if eval_mode == "both":
		if unseg == True:
			...
			loss, accuracy, auc_score, precision, recall, sensitivity, specificity = evaluate_event(net, criterion, device, data_gen, True, use_sim, generator, permutate)
		
		print('Loss: {:.4f} | Accuracy: {:.4f} | AUC: {:.4f}| Precision: {:.4f}, Recall: {:.4f} | Sensitivity: {:.4f}, Specificity: {:.4f}'.format(loss, accuracy, auc_score, precision, recall, sensitivity, specificity))
...
def detection_run(data_gen, device, model, model_save_path, eval_mode, use_sim=False, ref_genome=None, output_file="../output.tsv", unseg=False, generator=None):
	...
	if eval_mode == "both":
		if unseg == True:
			...
			loss, accuracy, auc_score, precision, recall, sensitivity, specificity = evaluate_event(net, criterion, device, data_gen, True, use_sim, generator)
...
if __name__ == "__main__":
	...
	parser.add_argument('--fast5_fold',default="", action="store",      type=str, required=False,  help="output file of the prediction results")
...
# todo add the pile-up evluation module
# generate a dictinoary of positions, pipe-up singals in the same position.
